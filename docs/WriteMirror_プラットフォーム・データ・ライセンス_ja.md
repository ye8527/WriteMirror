...
リポジトリ内で Feng YE が作成したコード、テスト、スクリプトおよび文書は、個別に別の表示がある場合を除き Apache License 2.0 で提供します。第三者のプラットフォーム、ライブラリ、データセット、およびKanjiVG由来のモデル・データをApache-2.0へ変更するものではありません。詳しい適用範囲と帰属は、リポジトリルートの `LICENSE`、`NOTICE`、`THIRD_PARTY_NOTICES.md` を参照してください。

## ライセンスの適用範囲

| 対象 | ライセンス・条件 |
|---|---|
| 作者が作成したソースコード、テスト、スクリプト、文書 | Apache License 2.0。個別ファイルに別の表示がある場合を除く |
| `trajectory-autoencoder-qdq-int8.onnx` とモデルカード | KanjiVG由来の成果物として CC BY-SA 3.0 |
| 再配布するKanjiVG由来の前処理データ、チェックポイント、派生モデル | CC BY-SA 3.0。帰属、変更表示、継承条件を維持 |
| NuGet/Python依存関係、Windows/Surface/QNN等 | 各提供元のライセンス、製品条件、第三者通知に従う |

Apache-2.0のルートライセンスは、KanjiVG、派生モデル、第三者ライブラリ、OS機能、ハードウェアサービスを再許諾しません。
...
- 発表では、作者実装、OS機能、学習データ、派生モデル、第三者ライブラリを区別して説明します。
- AI再構成差は診断、採点、能力、困難、改善または教育効果を示す値として使用しません。
- ソースまたはバイナリを再配布する場合は、対象に応じて `LICENSE`、`NOTICE`、`THIRD_PARTY_NOTICES.md`、モデルカードを同梱します。
